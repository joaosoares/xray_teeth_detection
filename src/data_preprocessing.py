...
class Preprocessor():
    ...
    @classmethod
    def bilateral_filter(cls, img, diameter, sigma_color, sigma_space, times):
        filtered = img
        for i in range(times):
            filtered = cv2.bilateralFilter(img, d=diameter, sigmaColor=sigma_color, sigmaSpace=sigma_space)
        return filtered
...
if __name__ == '__main__':
    ...
    img = Preprocessor.bilateral_filter(img, diameter=9, sigma_color=150, sigma_space=150, times=1)
